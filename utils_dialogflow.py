...
def process_infobot_webhook_request(
    ...
    if parameters.get("datastore_prompt"):
        search_query = parameters.get("datastore_prompt")    
    elif request.text:
...
def format_infobot_webhook_response(
    ...
    message_text.text = response_message.ResponseMessage.Text(text=[response.replace("https://storage.googleapis.com/", "https://storage.cloud.google.com/")])
    ...
    "richContent": [
        [
        {
            ...
            "actionLink": result.get("link").replace("https://storage.googleapis.com/", "https://storage.cloud.google.com/")
        }
